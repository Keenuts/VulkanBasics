...
void render_create_cmd(vulkan_info_t *info, vulkan_frame_info_t *frame) {
	...
	vkCmdBeginRenderPass(frame->command, &pass_begin_info, VK_SUBPASS_CONTENTS_INLINE);
	vkCmdBindPipeline(frame->command, VK_PIPELINE_BIND_POINT_GRAPHICS, info->pipeline);
	...
	render_end_command(&frame->command);
}
